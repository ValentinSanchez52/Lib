...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/14 04:47:56 by vsanchez          #+#    #+#             */
...
static inline unsigned long long	set_nb(t_attributes *att_list)
{
...
inline char							*ft_u_conv(t_attributes *att_list)
{
	char				*str;
	unsigned long long	nb;

	nb = set_nb(att_list);
	if (nb == 0 && att_list->prec && att_list->precision == 0)
		return (0);
	str = ft_ulltoa_base(nb, U_BASE);
	if (att_list->prec && att_list->precision)
